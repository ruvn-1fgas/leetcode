...
Status is an ENUM (category) type of ('completed', 'cancelled\_by\_driver', 'cancelled\_by\_client').
```
...
| role        | enum     |
+-------------+----------+
users\_id is the primary key (column with unique values) for this table.
